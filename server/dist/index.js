...
app.post('/api/items', requireAdmin, upload.array('images', 50), async (req, res) => {
    try {
        ...
        if (categoryIds && categoryIds.length > 0) {
            const rows = categoryIds.map((cid) => ({ item_id: data.id, category_id: cid }));
...
app.get('/api/categories/:id/items', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase
            ...
            .select('item_id')
            .eq('category_id', req.params.id)).data?.map((r) => r.item_id) || [])
            .order('order', { ascending: true })
            .order('created_at', { ascending: false });
        if (error) {
            res.status(500).json({ error: error.message });
...
app.get('/api/items/:id/categories', async (req, res) => {
    try {
        ...
        const { data, error } = await supabase
            ...
            .from('item_categories')
            .select('category_id')
            .eq('item_id', req.params.id)).data?.map((r) => r.category_id) || [])
...
app.get('/api/headcategories/:id', async (req, res) => {
    try {
        ...
        if (!data) {
            res.status(404).json({ error: 'Headcategory not found' });
            return;
        }
        res.json(data);
    }
