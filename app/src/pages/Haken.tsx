  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Filter categories and headcategories with type 'haken'
        const hakenCategories = catsData.filter((cat: Category) => cat.type === 'haken')
        const hakenHeadcategories = headcatsData.filter((headcat: HeadCategory) => headcat.type === 'haken')
  ...
  return (
    <div>
      ...
      <section 
        ...
        style={{ 
          backgroundImage: `url(${HakenBanner})`, 
      ...
      <div>
        ...
        >
              ...
              <div className="carousel">
                ...
                {[...items, ...items].map((item, idx) => {
                  ...
                  const href = category?.headcategory_id 
                    ? `/werkjes/haken/${headcategories.find(hc => hc.id === category.headcategory_id)?.slug}/${category.slug}/${itemSlug}`
                    : category ? `/werkjes/haken/${category.slug}/${itemSlug}` : '#'
          ...
          {!loading && !error && items.length === 0 && (
            <p className="text-center text-gray-500">Nog geen haken items beschikbaar.</p>
          )}
          </div>
        </section>
      </div>
