...
import HakenBanner from '../assets/banners/haken_banner.png'; // Fallback to Haken banner for now or replace if Combi banner exists
import WolligeAchtergrond from '../assets/haken/wollige_achtergrond.png'; // Fallback background
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                // Filter categories and headcategories with type 'combi'
                const combiCategories = catsData.filter((cat: Category) => cat.type === 'combi')
                const combiHeadcategories = headcatsData.filter((headcat: HeadCategory) => headcat.type === 'combi')

                // Load items from all combi categories
                const allItems: Item[] = []
                const categoryItemsMap: { [key: number]: Item[] } = {}

                for (const cat of combiCategories) {
                ...
                for (const headcat of combiHeadcategories) {
                ...
                // Get featured items for combi (fallback to sorted latest if request fails or not implemented yet)
                let featured: Item[] = []
                try {
                    const featRes = await apiFetch('/items/featured?type=combi')
                ...
                if (!cancelled) {
                    setCategories(combiCategories)
                    setHeadcategories(combiHeadcategories)
    ...
    return (
        <div>
            ...
            <section
                ...
                style={{
                    backgroundImage: `url(${HakenBanner})`, // TODO: Replace with Combi banner
            ...
            <div>
                            ...
                            {!loading && categories
                                ...
                                .map((category) => {
                                    ...
                                    return (
                                        <Link
                                            key={category.id}
                                            to={`/werkjes/combi/${category.slug}`}
                                            className="group block rounded-2xl overflow-hidden border border-gray-200 bg-white shadow-md hover:shadow-xl transition-shadow"
                                        >
                                            <div className="aspect-[940/788] bg-gray-100 overflow-hidden">
                                                {firstImage ? (
                ...
                >
                    <div className="mx-auto max-w-6xl">
                                ...
                                <Marquee speed={35} gradient={false} pauseOnHover={true} autoFill={false}>
                                    {[...items, ...items].map((item, idx) => {
                                        ...
                                        const href = category?.headcategory_id
                                            ? `/werkjes/combi/${headcategories.find(hc => hc.id === category.headcategory_id)?.slug}/${category.slug}/${itemSlug}`
                                            : category ? `/werkjes/combi/${category.slug}/${itemSlug}` : '#'
                        ...
                        {!loading && !error && items.length === 0 && (
                            <p className="text-center text-gray-500">Nog geen combi items beschikbaar.</p>
                        )}
                    </div>
                </section>
            </div>
